# Execution Registry

This file is auto-generated by `generate_registry.py`. Do not edit manually.
Run `python execution/generate_registry.py` after adding or modifying any script.
Last updated: 2026-04-30

## Scripts

| Script | Description | Inputs | Outputs |
|--------|-------------|--------|---------|
| `content\wedding_card_generator.py` | Generate a Karnataka Royal Heritage wedding invitation card PDF (Sowjanya & Ruturaj) | No CLI args — names and event details are hardcoded in the script | output/wedding_card_sowjanya_ruturaj.pdf (or similar in output/) |
| `enrichment\anymailfinder_lookup.py` | Find business owner emails via AnymailFinder API. | --input, --output, --min-confidence, --mock; env: ANYMAILFINDER_API_KEY | .tmp/enriched_leads.json |
| `enrichment\million_verifier.py` | Verify email deliverability via Million Verifier API. | --input, --output, --accept, --mock; env: MILLION_VERIFIER_API_KEY | .tmp/verified_leads.json |
| `gtm_client_workflows\accessory_masters_pipeline.py` | End-to-end orchestration pipeline for Accessory Masters cold email system. | --config, --stage, --mock, --force, --poll-replies; env: multiple API keys | .tmp/pipeline_state.json, .tmp/personalized_leads.json |
| `lead_sourcing\prospeo_leads.py` | Source B2B leads via Prospeo API for niches not indexed on Google Maps. | --domain, --company, --config, --output, --mock; env: PROSPEO_API_KEY | .tmp/prospeo_leads.json |
| `lead_sourcing\serper_maps_scraper.py` | Scrape Google Maps business listings via Serper.dev Maps API. | --query or --config, --limit, --output, --mock; env: SERPER_API_KEY | .tmp/serper_leads.json |
| `personal_workflows\cv_builder.py` | Generate an ATS-optimized, French-language PDF CV for Debanjan Mazumdar | --company (str), --role (str) via CLI args | .tmp/cv_{company}_debanjan_mazumdar.pdf |
| `personal_workflows\cv_optimizer_agent.py` | Interactive CV optimizer — scores any CV against a job description, | interactive prompts (JD text/path, CV PDF path, company name) | .tmp/cv_opt_{company}_{lastname}.pdf, .tmp/cover_letter_{company}_{lastname}.pdf |
| `personalization\ai_opener_generator.py` | Generate personalized cold email first lines using Claude API. | --input, --output, --tone-config, --batch-size, --mock; env: ANTHROPIC_API_KEY | .tmp/personalized_leads.json |
| `personalization\variant_generator.py` | Generate AI challenger email variants and analyze per-variant performance for A/B testing. | --action (generate|analyze|recommend|report), --config, --mock; env: ANTHROPIC_API_KEY, INSTANTLY_API_KEY | Updated config/email_variants.json, .tmp/variant_report.json |
