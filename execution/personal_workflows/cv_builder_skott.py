...
# ── Fonts (skott registers italic face) ────────────────────────────────────────
FONT, FONT_BOLD = _register_fonts(with_italic=True)

SEP = ' • '   # " • "
DOT = ' · '   # " · "


# ── Section header (SKOTT-style: navy uppercase title over a thick rule) ────────
...
# ── Skott-specific helper: inline-coloured skill line (not a table) ─────────────
def skill_line(cat: str, vals: str) -> Paragraph:
    """Single-line skill entry with a coloured bold category label."""
    return Paragraph(f'<font color="#1F4E79"><b>{cat} :</b></font> ' + vals, S['skill'])


# ── Variant-local exp_entry wrapper ────────────────────────────────────────────
def _exp_entry(title, company_line, bullets):
    # Skott: whole entry is atomic (KeepTogether wraps everything), no first-bullet trick
    return exp_entry(
...
def build_cv(output: Path):
    build_cv_doc(
        output, build_story(),
        margin_lr=MARGIN_LR, margin_tb=MARGIN_TB,
