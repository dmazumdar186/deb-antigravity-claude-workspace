...
| Discovery donut all-zero | `source_split.values = [0, 0, 0, 0]` across every range (7d / 30d / all) even though CFWA source is reported healthy | Real data-layer bug found by the 2026-08-05 acceptance gate run under `--strict`. Site definitely has traffic (34 WA taps in 7d = 34 people loaded the page and clicked the WhatsApp button), so CFWA `visits=0` is not a genuine empty-state. Suspect: `resolveSiteTag()` in `execution/infrastructure/yoga_jitendra_cron/src/sources/cf_wa.js` uses `env.CF_WA_SITE_TAG` when set (path 1, no roundtrip), so if the hardcoded value drifts from the actual account's beacon token the GraphQL query returns 0. Alternative: CFWA GraphQL might attribute traffic to a different siteTag than what's pinned. GSC + Bing 0 impressions/clicks is separately plausible for a new low-authority site. | Trigger `POST /run/cfwa` on the cron Worker with `X-Worker-Secret`, inspect the returned snapshot vs. Cloudflare's Web Analytics dashboard for the same date. If CF UI shows non-zero for the date the cron pulled, the resolver/query is the bug. Deferred — new investigation workstream. |
