...
**Written:** 2026-08-19, end of session 1
**Deadline:** Thursday 20 August 2026
**Status:** ~55% built. Core evidence engine verified on real data. NO DELIVERABLE ARTIFACT YET.
...
L8 adversarial pass, L9 contact enrichment, L10 movability, L11 messages,
L12 link liveness, **L13 renderer**, run.py orchestrator, pool maps,
client-side sidebar, Cloudflare Pages privacy notice, CSV export.

Nothing is committed to git yet.

## 3. Six corrections to SPEC.md (carry these forward — they cost hours to find)
...
- Authorised paid APIs: Anthropic, Serper, Firecrawl, Tavily, plus a new
  enrichment signup if needed. PROSPEO chosen for contact enrichment.
...
The SPEC is authoritative on WHAT to build. The HANDOFF carries six
...
publicaccess, ACP renamed, heredocs corrupt regex escapes).
...
count, never by loosening the L6 validator. If top tier cannot reach 10+5
after real source expansion, say so with pool-map numbers instead of padding.

BUILD ORDER
 1. run.py orchestrator + L13 renderer FIRST, so an artifact exists early and
    every later change is visible in the thing the client actually reads.
 ...
 3. L8 blind adversarial pass, L9 PROSPEO enrichment with honest status
    labels, L10 movability, L11 messages with injected GDPR strings,
    L12 link liveness + name match.
...
tests/fixtures/. The hallucination test must run on every commit.
...
CONSTRAINTS
Anthropic is funded; providers.autoselect_plan() picks the plan. Authorised:
Anthropic, Serper, Firecrawl, Tavily, PROSPEO. Never TOBIN/AtkinsRealis
candidates. Never AM-locked paths or keys. EUR for operator-facing figures.
Ask before git push. Use `py` on Windows. Use Write/Edit for files with regex
escapes, never bash heredocs.
```
