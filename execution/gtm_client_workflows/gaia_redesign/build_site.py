...
SECTOR_ART = {
    ...
    + '<path d="M0 156 C 40 152 80 160 120 156 S 200 148 260 154 L320 158 L320 200 L0 200Z" fill="#16283b" fill-opacity=".85"/>'
    + '<path d="M0 156 C 40 152 80 160 120 156 S 200 148 260 154 L320 158" stroke="#4fd066" stroke-opacity=".7" stroke-width="1.4"/>'
    + '<g stroke="#4fd066" stroke-opacity=".35" stroke-width="1">'
    ...
    for index, job in enumerate(jobs):
        ...
        if not isinstance(sectors, list) or not sectors:
            ...
            for sector in sectors:
                if "|" in str(sector):
                    problems.append(f"{slug}: sector {sector!r} contains '|', which collides with data-sector's separator")
        title = str(job.get("title") or "")
        if "<" in title or ">" in title:
            problems.append(f"{slug}: title contains '<' or '>': {title[:60]!r}")
        if slug in seen:
...
def _role_row(job: dict[str, Any], *, board: bool) -> str:
    ...
    data = ""
    if board:
        haystack = " ".join([job["title"], job["location"], " ".join(sectors), job["type"], job.get("summary") or ""])
        data = (
...
def sector_map_term(keywords: list[str], jobs: list[dict[str, Any]]) -> tuple[int, str]:
...
def render_sector_map(jobs: list[dict[str, Any]]) -> str:
    ...
    for name, keywords in SECTORS:
        count, term = sector_map_term(keywords, jobs)
...
def run_node_tests(src: Path) -> tuple[int, int] | None:
    ...
    # `node --test` can exit non-zero (e.g. a crash before any test ran) with
    # no "# fail" line to show for it, and it can also print a clean (0, 0)
    # summary if the test file loaded but matched nothing. Both are build
    # failures, never a silent "0 tests, all passing".
...
def guard_output(src: Path, out: Path) -> str:
    ...
    # Only ever replace a directory this script built, or one that does not
    # exist yet. Anything else (a source tree, someone's home) is refused.
    if out.exists():
        if not out.is_dir():
            return f"--out {out} exists and is not a directory"
        stamp = out / ".gaia-build"
        if not stamp.exists() and any(out.iterdir()):
            return (
                f"--out {out} is not empty and was not created by this build "
                f"(no {stamp.name} marker); refusing to delete it. If this "
                f"directory really is a stale/interrupted build of this same "
                f"pipeline, either remove it manually first or create an empty "
                f"{stamp.name} file in it to mark it as ours, then re-run."
            )
    return ""
...
def build(src: Path, out: Path, *, skip_links: bool) -> int:
    ...
    # Marks this directory as ours to replace on the next run. Written before
    # copytree populates it, so a build interrupted mid-copy still leaves the
    # marker behind and the next run can clean up and retry instead of being
    # bricked by guard_output's "not empty, not ours" refusal.
    (out / ".gaia-build").write_text("generated by build_site.py\n", encoding="utf-8")
    shutil.copytree(
        src,
        out,
    ...
    print(f"      html {html_bytes / 1024:.1f} KB · css {css / 1024:.1f} KB (budget 45) · js {js / 1024:.1f} KB (budget 32)")
    print(f"      site: {out}")
    return 0
