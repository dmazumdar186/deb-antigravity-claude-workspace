...
# Fix 7 — SSRF guard: only allow real YouTube URLs
...
def fetch_metadata_only(url: str) -> dict:
    ...
    Fix 13: skip-download for metadata on gemini-direct branch.
    """
    try:
        import yt_dlp
    except ImportError as e:
...
def extract_frames_at_timestamps(
    ...
    if not timestamps:
        ...
        # Estimate duration via ffprobe-style call; use max_frames * 10s as upper bound
...
def tile_frames_into_grids(
    ...
    # R2-2: import ImageDraw for placeholder rendering on paste failure
    try:
    ...
    for g in range(n_grids):
        ...
        for idx, (frame_path, _ts) in enumerate(batch):
            ...
            try:
                ...
                # R2-2: render a visible placeholder instead of leaving the cell black
...
# --------------------------------------------------------------------------- #
# Step 6a: Claude analysis with tool-use + caching  (replaces analyze_with_claude)
# --------------------------------------------------------------------------- #
...
def analyze_with_claude_v2(
    ...
    # Tool with cache_control on the static tools block (via _to_anthropic_tool_format)
    tools_payload = [
...
def analyze_with_gemini(url: str, metadata: dict, model_id: str) -> dict:
    ...
    try:
        ...
        # Best-effort: return a dict with raw text as hook
        return {
            "hook": cleaned[:500],
            "pacing_cuts": [],
...
def main() -> int:
    ...
    # Fix 2 — validate --max-frames range
    ...
    # R2-3 — full tier/provider cross-validation matrix
    # Valid (tier, provider) combos:
    ...
    # Model resolution
    # ------------------------------------------------------------------
    # R2-4 — shallow dry-run must make no network calls.
    # Pass allow_network=False when --dry-run is set; the registry uses cache →
    # LAST_KNOWN_GOOD without hitting the OR /models endpoint or any provider API.
    _allow_net = not args.dry_run

    if args.model:
        # Escape hatch: user-supplied exact model ID
        model_id = args.model
        log.info("Using user-supplied model override: %s", model_id)
    else:
        if provider == "openrouter":
            # For --tier gemini with OR: use the 'gemini' sub-tier in the registry
    ...
    # ------------------------------------------------------------------
    # Fix 11 — Honest cost projection via a small pricing lookup table
    # ------------------------------------------------------------------
    ...
    if provider == "openrouter":
        if tier == "gemini":
            cost_note = f"paid frame-grid mode via OR -- NOT free URL-native"
        else:
            cost_note = f"paid via OR (incl. 5.5% OR fee)"
    elif provider == "anthropic":
    ...
    # ------------------------------------------------------------------
    # SSRF guard (Fix 7) — validate YouTube host before any network call
    # ------------------------------------------------------------------
    ...
    # Fix 9 / R2-1 — validate --obsidian-vault is absolute BEFORE calling resolve()
    # (Path.resolve() always returns absolute, so the check must happen first)
    ...
    # ------------------------------------------------------------------
    # Fix 4 — SHALLOW dry-run gate BEFORE any network calls
    # ------------------------------------------------------------------
    if args.dry_run:
        # Estimate a rough cost without real token counts
        est_grids_guess = max(1, args.max_frames // 9)
        est_visual_tokens_guess = est_grids_guess * 1400   # Fix 12: calibrated to ~1400/grid
    ...
    if provider == "gemini-direct":
        # Fix 13 — use metadata-only fetch (no video download)
    ...
    # Fix 12 — calibrated to ~1400 tokens/grid (was 750)
    # Each 384x216 grid cell; Anthropic actual measurement ~155 tokens/cell x 9 = ~1400/grid
    est_visual_tokens = grid_count * 1400
    est_transcript_tokens = len(transcript_text) // 4
    est_total_tokens = est_visual_tokens + est_transcript_tokens + 500  # system + prompt overhead

    # Fix 4 — --deep-dry-run: full pipeline without AI call (old dry-run behavior)
    ...
    # Fix 8 — wrap unlink() in try/except for Windows PermissionError
    if not args.keep_source and video_path.exists():
        try:
            video_path.unlink()
            log.info("Deleted source .mp4 to save disk.")
