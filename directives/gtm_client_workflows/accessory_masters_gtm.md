...
| 2026-05-12 | Auto-reply engine expanded to 11-entry broker objection playbook (was 3). Added `voice_reference_file` injection (`config/accessory_masters_voice.md`) and `cta_variants` rotation in `auto_reply` config to kill the verbatim "Are you free tomorrow for a quick 10 minute call?" tell. New guard rail: vary the CTA wording. New directive `directives/personalization/auto_reply.md` documents the engine. 19/19 tests in `tests/test_flow_chains.py` passing. |
