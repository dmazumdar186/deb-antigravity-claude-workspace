...
- **Splash screen** at `assets/splash.png` — `2048x2048` PNG (Expo's recommended single-asset square; scales with `resizeMode: "contain"` across iPhone + iPad). Solid background colour matches `app.json` `splash.backgroundColor`. Do NOT use device-screen pixel dimensions (e.g. 1284x2778 = iPhone 14/15 Pro Max screen size) as the asset — Expo will crop or letterbox on every other device.
