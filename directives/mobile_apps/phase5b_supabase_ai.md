...
3. **Implement the function.** TypeScript / Deno (uses `Deno.serve`, the current Supabase Edge runtime entrypoint — `std/http/server` is deprecated and will eventually be removed):
   ...
   ```
   Note: `Deno.serve` returns a `Promise<HttpServer>` that resolves only on server shutdown. The handler is registered immediately, so request handling starts at the call site. You CAN `await` it if you want code to run on shutdown, but in a Supabase Edge Function there's nothing useful to do after shutdown — keep it unawaited at the top level of the function module.
...
- **Em-dash leakage.** Nick chapter 22 specifically calls out em-dashes (`—`) as an AI tell. Add `no em-dashes` to the system prompt explicitly. The model still slips occasionally — post-process with `text.replace(/[‐-―−﹘﹣－]|--+/g, ', ')`. Covers U+2010-U+2015 (hyphen through horizontal bar, incl. figure dash, en-dash, em-dash), U+2212 minus, U+FE58 small em-dash, U+FE63 small hyphen-minus, U+FF0D fullwidth hyphen-minus, and ASCII `--+`. **Do NOT use `\p{Dash}`** — per Unicode PropList.txt, U+002D HYPHEN-MINUS has `Dash=Yes`, so `\p{Dash}` over-matches: it strips every `-` in normal English (`well-known` → `well, known`). Explicit enumeration is the right altitude here precisely because the Unicode property is too broad for the intent ("strip AI-style dashes, keep ordinary hyphens").
