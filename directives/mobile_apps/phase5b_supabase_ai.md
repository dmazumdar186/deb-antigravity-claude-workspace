...
- **Em-dash leakage.** Nick chapter 22 specifically calls out em-dashes (`—`) as an AI tell. Add `no em-dashes` to the system prompt explicitly. The model still slips occasionally — post-process with a regex that catches the entire General Punctuation dash block + minus-like codepoints, not just U+2014. Safer pattern: `text.replace(/[‐-―−﹘﹣－]|--+/g, ', ')` covers U+2010 hyphen through U+2015 horizontal bar (incl. figure dash, en-dash, em-dash), U+2212 minus, U+FE58 small em-dash, U+FE63 small hyphen-minus, U+FF0D fullwidth hyphen-minus, and ASCII `--+`. Don't promise "every dash" — there are always edge codepoints; promise "all dashes in the Unicode dash classes Sonnet has been observed emitting".
