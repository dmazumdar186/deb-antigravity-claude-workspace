...
- **Em-dash leakage.** Nick chapter 22 specifically calls out em-dashes (`—`) as an AI tell. Add `no em-dashes` to the system prompt explicitly. The model still slips occasionally — post-process with `text.replace(/\p{Dash}|--+/gu, ', ')`. The `\p{Dash}` Unicode property class (JS ES2018+, supported by Deno) matches every codepoint flagged with `Dash=Yes` in the Unicode Character Database — em-dash, en-dash, figure dash, horizontal bar, minus sign, small / fullwidth variants, future additions, all of them. The `|--+` alternative still strips ASCII double-hyphen. Don't enumerate codepoints by hand; the property class is the right altitude.
