'use strict';
/* Home film: a pinned hero scrubbed by scroll. One canvas of particles plays
   five states — wind field, the edition map (points sampled from the SVG at
   build time), the disclosed-salary bands, the sector ring, a calm field
   behind the real search box. Progress → state (15% hold, 70% travel);
   every particle eases to a per-state target (easeOutQuart, staggered by
   index) so a wheel flick stays smooth. DPR-aware, paused off-screen, three
   states under 900px, static with all copy under reduced motion or no data. */
(function () {
  ...
  function topFor(k) { /* scrollY at which state k (position in SEQ) is fully held */
  ...
  /* Keyboard: tabbing into a state's copy scrolls the film to that state, and
     the skip link jumps straight to the search box. */
