'use strict';
/* Home hero: a living landscape drawn on one canvas behind the copy. Layered
   hills (paper / sage / clay, hazed with depth), a river with a slow glint, a
   solar field on one slope, five wind turbines at their own speeds, a
   hedgerow, drifting clouds, the odd pair of birds and a low sun that warms as
   you scroll. Static layers are rasterised once per size into offscreen
   canvases and composited per frame with parallax offsets (scroll and pointer),
   so a frame is a handful of blits plus the moving parts. DPR-aware, paused
   off-screen, one static frame under reduced motion, CSS gradient without
   canvas. If a drop-in <video> is present it takes over once its first frame
   is ready; the canvas keeps playing until then. */
(function () {
  ...
  if (!ctx) return; /* the CSS gradient on .hero__scene is the scene */
  ...
  var TURB = [[0.16, 62, 0.9], [0.24, 78, 1.15], [0.33, 56, 0.75], [0.71, 70, 1.0], [0.8, 52, 0.85]]; /* x fraction, tower height, rad/s */
  ...
  /* Smooth ridge line: layered sines, deterministic per layer seed. */
  ...
  function build() {
    ...
    L.mid = layer(function (g) {
      ...
      /* river: from the mid ridge down and left, widening */
    ...
    L.near = layer(function (g) {
      ...
      /* solar field on the right-hand slope */
    ...
    L.nearest = layer(function (g) {
      ...
      /* hedgerow: a line of round trees along the crest */
  ...
  function draw(now) {
    ...
    /* clouds drift right to left, wrapping */
    ...
    /* hills, back to front, each at its own parallax rate */
    ...
    /* river glint: a travelling dash along the same curve */
