...
**(c) Monday: shortlist + Brief Controls + v2 page only. The check page is held for the follow-up call.** Reasons: Brief Controls already delivers "you were right on both" on the August list, and a third re-cut of the same 13 names in one email reads as scoring points; the check page carries euros, and the email deliberately carries no price; and once Keith's two threshold answers arrive, the check page is re-run under his own brief and lands with the pricing conversation, where it belongs. Maddie's engineering notes go to Maddie only after she confirms the stage boundary (item 5), via Keith. `monday_send.md` is unchanged except for one line recording this.

**Also built on day 2, at zero model spend:** the three LOW items (an "over N years" figure now counts as above N against a ceiling; the validator folds the OCR confusion 1/l/I so Pearse Sutton's "I have over 40 years" quote validates; `august_list` no longer carries unused defaults), the score-versus-check line on Keith's page, and the day-2 re-validate of the cached corpus (offline) with the acceptance gate re-run. Numbers changed by the re-validate are recorded below once the run is in.
