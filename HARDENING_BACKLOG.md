...
**Audit stack outcome (same day, after the first commit `e016643`):** four real sub-agents (Karpathy / Cherny / Amodei / research lenses) + `pipeline-auditor` + anneal ran on the diff. Acted on every finding:
- `interview_iag/tests/front_door_agentup.sh` still asserted `claude-sonnet-4-6` -> fixed; live front-door PASSes against production (`primary_model: claude-sonnet-5`, real Claude round-trip). Production was promoted with `--branch main` (the first `npm run deploy` had landed as a branch preview).
- 6 more pins the first regex could not see: `claude-sonnet-4-20250514` (modal_webhook x2, classify_leads_llm x2), `claude-3-5-haiku-20241022` (modal_webhook, casualize_batch), `anthropic/claude-sonnet-4.6` (`execution/modules/llm_client.py` shared default). All -> 5-series.
- `legacy-model-pin` regex widened (OR `anthropic/` prefix, dot separators, dated `sonnet-4-2025...`, 3.x Haiku, `model: str = ` annotations); comment-based suppression removed. `haiku-banned` regex now covers the 3.5 family. **6 fixture tests** added in `tests/test_workspace_sast_rules.py` (rule-backport-cadence: guardrail + test).
- `model_registry.py`: Haiku removed from both family ladders and from the OpenRouter default ladder; `tests/test_model_tier_guardrails.py` asserts no ladder contains haiku and offline resolution is fable-5/sonnet-5.
- gaia L10 (movability) moved back to `claude-sonnet-5`: per-candidate rubric scoring is bulk execution under model-tier.md Exhibit D. L8 + L11 stay on Fable 5. Cost quantified in `config.py`: ~76 Fable calls per 38-candidate run = ~EUR 9 vs ~EUR 4.5 on Opus 5. `test_providers_and_cache.py` now pins "unknown model = dearest model" against the whole table, not Opus.
- `sonnet_rerank.py` priced at $3/$15 with a Sonnet 5 default -> $2/$10 (was over-reporting 1.5x). Help text in `_TEMPLATE*.py`, `ai_opener`, `variant_generator`, `anthropic_watch`; docs in gmaps SKILL.md, cv_optimizer_local.md, mobile_apps phase5a/5b + app_design, `_TEMPLATE_autoresearch.md`, glm_5_2_integration.md, dynamic-workflows/sub-agent-delegation rules, model_registry docstrings, anneal HANDOFF/README/examples.
- OpenRouter catalogue checked live: `anthropic/claude-fable-5` is listed (gaia's default plan will resolve).
- Karpathy lens: nothing committed *pinned* the new tier map (green suites only proved nothing broke). `tests/test_model_tier_sweep.py` (12 tests) now asserts every swept literal, gaia roles + EUR pricing, registry premium, youtube/humanizer price rows and the rerank prices; verified to FAIL 11/12 on the pre-sweep tree `6fa4357` and pass on HEAD. The youtube e2e premium assertion is back to `'fable' in model_id` only. `legacy-model-pin` also accepts the `model: str = "..."` annotated shape (the one `llm_client.py` used).

**Owed / not done:**
- **Gated on spend approval** (CLAUDE.md: paid tokens need the operator's OK): (a) a measured anneal cost-per-round on `examples/synthetic_buggy` at `--tier balanced` (~EUR 0.25) -- README figures are labelled estimates until then; (b) one paid `messages.create(model="claude-fable-5")` from `execution/` and one OpenRouter call to `anthropic/claude-fable-5` (~EUR 0.01). Sonnet 5 IS verified live (agentup-iag front-door round-trip); Fable 5 is verified live only through Claude Code sessions.
