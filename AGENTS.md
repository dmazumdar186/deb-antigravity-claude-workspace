...
- **Orchestrator / brain: `claude-fable-5-1`** — the session default. It plans, architects, decides, reviews diffs, and delegates. It does **not** grind: exploration, implementation from an approved plan, scraping, formatting, and fan-out all go to worker sub-agents. Every main-session token should be a judgement token. 5.1 cache reads are $0.25/MTok: the re-sent prefix is cheap only while it stays byte-stable.
- **Workers / execution: `claude-fable-5`** (same $10/$50 MTok as 5.1; operator found Sonnet workers too weak, 2026-09-21). All Explore/general-purpose sub-agents, Dynamic Workflow workers, execution scripts' default tier, mechanical agents (documenter, note-taker), and checklist audits (anneal-reviewer, qa, code-reviewer). Workers run at `effort: low` for mechanical work, `medium` for implementation; only the brain runs high.
- **Audit lens:** `pipeline-auditor` = `claude-fable-5-1` (adversarial verification deserves the brain).
- Cost control = context diet + delegation + hygiene + effort tiers (`.claude/rules/token-economy.md`) — never downgrading the brain. Sonnet/Opus/Haiku are not used. Pin full model IDs, never aliases. History + reverts: `.claude/SETTINGS_NOTES.md`. In `execution/`, tiers resolve via `model_registry.LAST_KNOWN_GOOD` (`'default'` = Fable 5; `'premium'` = Fable 5.1).
