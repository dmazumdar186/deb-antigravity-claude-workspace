...
| Dynamic Workflow (`ultracode:`) | 5–16+ independent fan-out tasks, long jobs, runs in background | up to 16 concurrent, 1000 total | Out-of-process | Medium (claude-sonnet-5 workers per user-level model-tier rule; Haiku 4.5 is banned) |
...
- Note: Fable 5 access was suspended 2026-06-12 (US export-control directive). The operator re-enabled it as the session default on 2026-08-27. **Verified live 2026-08-27** (session `9410a201` ran on `claude-fable-5`, no model-access error) — if a future session fails with one, revert with the one-liner in `.claude/SETTINGS_NOTES.md`. Fable 5 is $10/$50 per MTok — 2x Opus 5, 5x Sonnet 5 — which is why the mundane-execution boundary matters.
- **Judgement tier: `claude-fable-5`** (set 2026-08-27, replacing `claude-opus-5`). The operator's rule is blunt: everything that is not mundane execution is Fable 5 — reasoning, planning, architecture, analysis, audit, review, ambiguous judgement. Resolves via `model_registry.LAST_KNOWN_GOOD[...]['premium']`.
- **Execution tier: `claude-sonnet-5`** ($2/$10 per MTok) — mundane per-row work, classification, extraction, formatting. Unchanged. Resolves via the `'default'` tier.
- `claude-opus-5` is no longer a tier target. It survives only as the explicit `opus` alias in `model_router.ALIASES` for deliberate one-off selection, and is kept in `model_registry._ADDITIONAL_KNOWN` so the registry-coupling rule still validates that alias.
- Implementation / exploration sub-agents: `claude-sonnet-5` (execution work). Analysis sub-agents — `anneal-reviewer`, `code-reviewer`, `pipeline-auditor`, `qa` — are pinned to `claude-fable-5` in their `.claude/agents/*.md` frontmatter; `documenter` and `note-taker` stay on `claude-sonnet-5`.
- High-volume fan-out workers (Dynamic Workflows / agent teams of N parallel workers): `claude-sonnet-5`. (Per `~/.claude/rules/model-tier.md` — Haiku 4.5 is BANNED for any project work, including fan-out. The user-level rule wins over any workspace default.)
- Per-agent model override: `model:` frontmatter in `.claude/agents/*.md`. Full model IDs (`claude-fable-5`, `claude-sonnet-5`) are accepted — the field takes the same values as `--model` (confirmed against code.claude.com/docs/en/sub-agents, 2026-08-27).
