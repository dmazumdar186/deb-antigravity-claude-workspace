...
- [pattern] Live-data tuning: after building a deterministic filter, run the pipeline live in --dry-run and inspect every rejection/keep sample by hand — the 2026-09-10 domain filter had two false-positive classes that no unit corpus anticipated.
...
- [learned] wrangler 4.136+ (2026-09-22): `pages project create` is delegated to Workers and fails with "Missing entry-point"; pass `--force` for a classic Pages project. Cloud sessions have no `.env` — only env vars set before the container started (no ANTHROPIC key was present on 2026-09-22 despite the local `.env` having one).
- [pattern] greenjobs_redesign (2026-09-22): a padded "employers hiring now" logo strip named third parties with zero live roles — the panel caught it, the validator now asserts every "hiring" logo has a live job. Any honesty claim about named third parties needs a build-time assertion, not a review.
- [constraint] Secrets location (operator, 2026-09-22): the canonical `.env` is `C:\Users\deban\OneDrive\Documents\AntiGravity Project Space\.env` (holds ANTHROPIC_API_KEY, the GLM/Z.ai key and FIRECRAWL_API_KEY as of 2026-09-22, plus the rest). Cloud sessions cannot read it; do not ask the operator about keys again — if a cloud task needs one, name the exact variable in HANDOFF.md and proceed with the non-key path. Local sessions load it with `load_dotenv()` as usual.
