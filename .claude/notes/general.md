...
- [constraint] Secrets location (operator, 2026-09-22): the canonical `.env` is `C:\Users\deban\OneDrive\Documents\AntiGravity Project Space\.env` (holds ANTHROPIC_API_KEY and the GLM/Z.ai key, plus the rest). Cloud sessions cannot read it; do not ask the operator about keys again — if a cloud task needs one, name the exact variable in HANDOFF.md and proceed with the non-key path. Local sessions load it with `load_dotenv()` as usual.
