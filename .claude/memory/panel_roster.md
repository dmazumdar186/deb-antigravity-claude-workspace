...
Rule of use (operator standing order 2026-09-16: nothing ships without every lens): before any "shipped/done" claim, every lens below is run as a real sub-agent (or a
...
| 7 | **Daniela Amodei** — operations, people & trust | Who is on the receiving end of every automated action, and would they feel respected if they saw the whole system? Is the operator's own reputation (domain, name, address in the footer) protected? Is there an owner for every alert and a quiet path for every complaint? | 2026-09-16 |
| 8 | **Ilya Sutskever** — capability & failure modes | Where does the model sit in the loop, what is the worst plausible output at each LLM call, and what bounds it (schema, lint, allowlist, human)? Are eval sets real, and would the system notice if the model got worse? | 2026-09-16 |
| 9 | **Mira Murati** — product & user experience | What does the prospect actually see (email, preview, remove flow) on a phone at 11pm, and does each touch feel like one person wrote it? Is the operator's daily surface (dashboard, Telegram, sheet) the smallest set of things they must look at? | 2026-09-16 |
| 10 | **Greg Brockman** — engineering execution & reliability | Can this run unattended for 30 days? Idempotent reruns, retries with bounded blast radius, secrets rotation, cron overlap, dead-man alarms, one-command redeploy, and a runbook a stranger can follow at 6am. | 2026-09-16 |
| 11 | **Demis Hassabis** — systems thinking & long-horizon strategy | Does the whole loop (discover, audit, build, send, reply, close, deliver) compound: does each metro make the next one cheaper and the scoring better? Which measurements feed back into which decisions, and which are dead ends? What is the 12-month shape if this works? | 2026-09-16 |

Lenses 7-11 added 2026-09-16 at the operator's request ("the Amodei siblings, Ilya Sutskever, Mira Murati, Boris Cherny, Andrej Karpathy, Greg Brockman and Demis Hassabis"); Dario Amodei stays lens 3.
