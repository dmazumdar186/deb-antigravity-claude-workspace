#!/usr/bin/env python3
"""
Recreate YouTube thumbnails with Debanjan Mazumdar's face using Nano Banana Pro.
...
4. Recreates it with Debanjan Mazumdar's face swapped in
...
def load_reference_photos(max_photos: int = 3, specific_path: Path | None = None) -> list[Image.Image]:
    """
    Load Debanjan Mazumdar's reference photos for face consistency.
...
def recreate_thumbnail(
    ...
    """
    Recreate a thumbnail with Debanjan Mazumdar's face and style variations.

    Args:
        source_image: Original thumbnail to recreate
        reference_photos: Reference photos of Debanjan Mazumdar for face consistency
    ...
    prompt = f"""IMAGE 1{' and 2' if num_refs > 1 else ''}: Reference photo(s) of Debanjan Mazumdar's face.
IMAGE {num_refs + 1}: The thumbnail to edit.

TASK: Replace ONLY the face in the thumbnail with Debanjan Mazumdar's exact face from the reference(s).

The output must be a 100% exact duplicate of the thumbnail, except the face is swapped with Debanjan Mazumdar's face. Keep every single pixel identical outside the face region.
...
def main():
    parser = argparse.ArgumentParser(
        description="Recreate YouTube thumbnails with Debanjan Mazumdar"
    )
    parser.add_argument(
        "--youtube", "-y",
        type=str,
